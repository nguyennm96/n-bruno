    ...
    <StyledWrapper className={`${isVerticalLayout ? 'vertical-layout' : ''}`}>
      ...
      <div className={`flex ${isVerticalLayout ? 'mt-2' : 'mt-4'}`}>
        <div className="flex flex-1 flex-col items-end px-1">
          <div className="px-1 py-2">{t('RESPONSE_PANE.PLACEHOLDER.SEND_REQUEST')}</div>
          <div className="px-1 py-2">{t('RESPONSE_PANE.PLACEHOLDER.NEW_REQUEST')}</div>
          <div className="px-1 py-2">{t('RESPONSE_PANE.PLACEHOLDER.EDIT_ENVIRONMENTS')}</div>
        </div>
