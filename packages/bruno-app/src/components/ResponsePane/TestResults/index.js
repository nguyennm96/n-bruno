...
const ResultIcon = ({ status, t }) => (
  <span className={`inline-flex items-center ${status === 'pass' ? 'test-success' : 'test-failure'}`}>
    {status === 'pass' ? (
      <IconCircleCheck size={14} className="mr-1" aria-label={t('RESPONSE_PANE.TEST_RESULTS.TEST_PASSED')} />
    ) : (
      <IconCircleX size={14} className="mr-1" aria-label={t('RESPONSE_PANE.TEST_RESULTS.TEST_FAILED')} />
    )}
  </span>
);
    ...
    <div className="mb-4">
      ...
      >
        ...
        <span className="flex-grow">
          {t('RESPONSE_PANE.TEST_RESULTS.SUMMARY', {
            title,
...
const TestResults = ({ results, assertionResults, preRequestTestResults, postResponseTestResults }) => {
  ...
  if (!results.length && !assertionResults.length && !preRequestTestResults.length && !postResponseTestResults.length) {
    return <div>{t('RESPONSE_PANE.TEST_RESULTS.NO_TESTS_FOUND')}</div>;
  }

  return (
    <StyledWrapper className="flex flex-col">
      <TestSection
        title={t('RESPONSE_PANE.TEST_RESULTS.PRE_REQUEST_TESTS')}
      ...
      <TestSection
        title={t('RESPONSE_PANE.TEST_RESULTS.POST_RESPONSE_TESTS')}
      ...
      <TestSection
        title={t('RESPONSE_PANE.TEST_RESULTS.TESTS')}
        results={results}
      ...
      <TestSection
        title={t('RESPONSE_PANE.TEST_RESULTS.ASSERTIONS')}
