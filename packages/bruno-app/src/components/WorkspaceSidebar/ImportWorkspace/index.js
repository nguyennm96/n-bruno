...
const ImportWorkspace = ({ onClose }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        toast.success(t('WORKSPACE.IMPORT_SUCCESS'));
        onClose();
      } catch (error) {
        toast.error(multiLineMsg(t('WORKSPACE.IMPORT_ERROR'), formatIpcError(error)));
      } finally {
  ...
  const validateAndGetFilePath = (file) => {
    ...
    if (!isZip) {
      toast.error(t('WORKSPACE.INVALID_ZIP_FILE'));
      return null;
    }

    const filePath = window?.ipcRenderer?.getFilePath(file);
    if (!filePath) {
      toast.error(t('WORKSPACE.FILE_PATH_ERROR'));
      return null;
    }
  ...
  return (
    <Modal
      size="md"
      title={t('WORKSPACE.IMPORT_TITLE')}
      confirmText={isSubmitting ? t('WORKSPACE.IMPORTING') : t('WORKSPACE.IMPORT')}
    ...
    >
      <div className="flex flex-col">
        <div className="mb-4">
          <h3 className="font-semibold mb-2">{t('WORKSPACE.WORKSPACE_FILE_LABEL')}</h3>
          {selectedFile ? (
        ...
        <div className="mb-4">
          <label htmlFor="workspace-location" className="font-semibold mb-2 flex items-center">
            {t('WORKSPACE.EXTRACT_LOCATION')}
            <Help>
              <p>
                {t('WORKSPACE.EXTRACT_LOCATION_HELP_1')}
              </p>
              <p className="mt-2">
                {t('WORKSPACE.EXTRACT_LOCATION_HELP_2')}
              </p>
            </Help>
          </label>
          <input
