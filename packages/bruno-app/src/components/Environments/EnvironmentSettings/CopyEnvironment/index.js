...
const CopyEnvironment = ({ collection, environment, onClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string()
        ...
        .max(50, t('ENVIRONMENTS.NAME_MAX_LENGTH_50'))
  ...
  return (
    <Portal>
      <Modal size="sm" title={t('ENVIRONMENTS.COPY_ENVIRONMENT')} confirmText={t('ENVIRONMENTS.COPY')} handleConfirm={onSubmit} handleCancel={onClose}>
        <form className="bruno-form" onSubmit={(e) => e.preventDefault()}>
          <div>
            <label htmlFor="name" className="block font-medium">
              {t('ENVIRONMENTS.NEW_ENVIRONMENT_NAME')}
            </label>
            <input
              id="environment-name"
              type="text"
