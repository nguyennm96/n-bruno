...
const RenameEnvironment = ({ onClose, environment }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string()
        .min(1, t('WORKSPACE_ENVIRONMENTS.NAME_MIN_LENGTH'))
        .max(255, t('WORKSPACE_ENVIRONMENTS.NAME_MAX_LENGTH'))
        .test('is-valid-filename', function (value) {
          const isValid = validateName(value);
          return isValid ? true : this.createError({ message: validateNameError(value) });
        })
        .required(t('WORKSPACE_ENVIRONMENTS.NAME_REQUIRED'))
        .test('duplicate-name', t('WORKSPACE_ENVIRONMENTS.ENVIRONMENT_ALREADY_EXISTS'), validateEnvironmentName)
    }),
    onSubmit: (values) => {
      ...
      dispatch(renameGlobalEnvironment({ name: values.name, environmentUid: environment.uid }))
        .then(() => {
          toast.success(t('WORKSPACE_ENVIRONMENTS.ENVIRONMENT_RENAMED_SUCCESSFULLY'));
          onClose();
        })
        .catch((error) => {
