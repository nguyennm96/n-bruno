        ...
        <div className="card shadow-sm rounded-md p-4 w-[685px]">
          <div>
            <div className="font-medium flex items-center gap-2 pb-4">
              ...
              {t('REQUEST_NOT_LOADED.fileInfo')}
            </div>
            <div className="hr" />

            <div className="flex items-center mt-2">
              <span className="w-12 mr-2 text-muted">{t('REQUEST_NOT_LOADED.name')}</span>
              <div>{item?.name}</div>
            </div>

            <div className="flex items-center mt-1">
              <span className="w-12 mr-2 text-muted">{t('REQUEST_NOT_LOADED.path')}</span>
              <div className="break-all">{item?.pathname}</div>
            </div>

            <div className="flex items-center mt-1 pb-4">
              <span className="w-12 mr-2 text-muted">{t('REQUEST_NOT_LOADED.size')}</span>
            ...
            {!item?.error && (
              <div className="flex flex-col">
                <div className="flex items-center gap-2 px-3 py-2 title bg-yellow-50 dark:bg-yellow-900/20">
                  ...
                  <span>{t('REQUEST_NOT_LOADED.notLoadedWarning')}</span>
                </div>
                <div className="flex flex-row mt-6 items-center gap-2 w-full">
                  ...
                  >
                    {t('REQUEST_NOT_LOADED.loadRequest')}
                  </button>
                  <p>{t('REQUEST_NOT_LOADED.regexApproach')}</p>
                </div>
              </div>
            )}
