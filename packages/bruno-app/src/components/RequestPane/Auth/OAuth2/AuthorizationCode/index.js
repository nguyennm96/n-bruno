...
const OAuth2AuthorizationCode = ({ save, item = {}, request, handleRun, updateAuth, collection, folder }) => {
  ...
  const handleUseSystemBrowserToggle = (e) => {
    ...
    dispatch(
      ...
      .then(() => {
        toast.success(t('REQUEST.AUTH.OAUTH2.PREFERENCE_UPDATED'));
      })
      .catch((err) => {
        console.error(err);
        toast.error(t('REQUEST.AUTH.OAUTH2.PREFERENCE_UPDATE_FAILED'));
      });
  };
    ...
    <StyledWrapper className="mt-2 flex w-full gap-4 flex-col">
      ...
      <div className="flex items-center gap-2.5 mt-2">
        ...
        <span className="oauth2-section-label">
          {t('REQUEST.AUTH.OAUTH2.CONFIGURATION')}
        </span>
      </div>
      <div className="flex items-center gap-4 w-full" key="input-callbackUrl">
        <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.CALLBACK_URL')}</label>
      ...
      <div className="flex items-center gap-4 w-full" key="input-use-system-browser">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {t('REQUEST.AUTH.OAUTH2.USE_SYSTEM_BROWSER')}
          </label>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 w-full" key="input-credentials-placement">
        <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.ADD_CREDENTIALS_TO')}</label>
        <div className="inline-flex items-center cursor-pointer token-placement-selector">
          <MenuDropdown
            items={[
              { id: 'body', label: t('REQUEST.AUTH.OAUTH2.REQUEST_BODY'), onClick: () => handleChange('credentialsPlacement', 'body') },
              { id: 'basic_auth_header', label: t('REQUEST.AUTH.OAUTH2.BASIC_AUTH_HEADER'), onClick: () => handleChange('credentialsPlacement', 'basic_auth_header') }
            ]}
            selectedItemId={credentialsPlacement}
            placement="bottom-end"
          >
            <div className="flex items-center justify-end token-placement-label select-none">
              {credentialsPlacement == 'body' ? t('REQUEST.AUTH.OAUTH2.REQUEST_BODY') : t('REQUEST.AUTH.OAUTH2.BASIC_AUTH_HEADER')}
      ...
      <div className="flex flex-row w-full gap-4" key="pkce">
        <label className="block">{t('REQUEST.AUTH.OAUTH2.USE_PKCE')}</label>
        <input
          className="cursor-pointer"
          type="checkbox"
      ...
      <div className="flex items-center gap-2.5 mt-2">
        ...
        <span className="oauth2-section-label">
          {t('REQUEST.AUTH.OAUTH2.TOKEN')}
        </span>
      </div>
      <div className="flex items-center gap-4 w-full" key="input-token-type">
        <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.TOKEN_SOURCE')}</label>
        <div className="inline-flex items-center cursor-pointer token-placement-selector">
          <MenuDropdown
            items={[
              { id: 'access_token', label: t('REQUEST.AUTH.OAUTH2.ACCESS_TOKEN_LABEL'), onClick: () => handleChange('tokenSource', 'access_token') },
              { id: 'id_token', label: t('REQUEST.AUTH.OAUTH2.ID_TOKEN'), onClick: () => handleChange('tokenSource', 'id_token') }
            ]}
            selectedItemId={tokenSource}
            placement="bottom-end"
          >
            <div className="flex items-center justify-end token-placement-label select-none">
              {tokenSource === 'id_token' ? t('REQUEST.AUTH.OAUTH2.ID_TOKEN') : t('REQUEST.AUTH.OAUTH2.ACCESS_TOKEN_LABEL')}
              <IconCaretDown className="caret ml-1 mr-1" size={14} strokeWidth={2} />
            </div>
          </MenuDropdown>
        </div>
      ...
      <div className="flex items-center gap-4 w-full" key="input-token-name">
        <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.TOKEN_ID')}</label>
      ...
      <div className="flex items-center gap-4 w-full" key="input-token-placement">
        <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.ADD_TOKEN_TO')}</label>
        <div className="inline-flex items-center cursor-pointer token-placement-selector">
          <MenuDropdown
            items={[
              { id: 'header', label: t('REQUEST.AUTH.OAUTH2.HEADER_LABEL'), onClick: () => handleChange('tokenPlacement', 'header') },
              { id: 'url', label: t('REQUEST.AUTH.OAUTH2.URL_LABEL'), onClick: () => handleChange('tokenPlacement', 'url') }
            ]}
            selectedItemId={tokenPlacement}
            placement="bottom-end"
          >
            <div className="flex items-center justify-end token-placement-label select-none">
              {tokenPlacement == 'url' ? t('REQUEST.AUTH.OAUTH2.URL_LABEL') : t('REQUEST.AUTH.OAUTH2.HEADERS_LABEL')}
              <IconCaretDown className="caret ml-1 mr-1" size={14} strokeWidth={2} />
            </div>
          </MenuDropdown>
        </div>
        ...
        tokenPlacement === 'header'
          ? (
              <div className="flex items-center gap-4 w-full" key="input-token-prefix">
                <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.HEADER_PREFIX')}</label>
                <div className="single-line-editor-wrapper flex-1">
                  <SingleLineEditor
          ...
          : (
              <div className="flex items-center gap-4 w-full" key="input-token-query-param-key">
                <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.QUERY_PARAM_KEY')}</label>
                <div className="single-line-editor-wrapper flex-1">
                  <SingleLineEditor
      ...
      <div className="flex items-center gap-2.5 mt-4 mb-2">
        ...
        <span className="oauth2-section-label">
          {t('REQUEST.AUTH.OAUTH2.ADVANCED_SETTINGS')}
        </span>
      </div>

      <div className="flex items-center gap-4 w-full mb-4">
        <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.REFRESH_TOKEN_URL')}</label>
        <div className="single-line-editor-wrapper flex-1">
          <SingleLineEditor
      ...
      <div className="flex items-center gap-2.5 mt-4">
        ...
        <span className="font-medium">{t('REQUEST.AUTH.OAUTH2.SETTINGS')}</span>
      </div>
      ...
      <div className="flex items-center gap-4 w-full">
        ...
        />
        <label className="block min-w-[140px]">{t('REQUEST.AUTH.OAUTH2.AUTO_FETCH_TOKEN')}</label>
        <div className="flex items-center gap-2">
          <div className="relative group cursor-pointer">
            ...
            <span className="group-hover:opacity-100 pointer-events-none opacity-0 max-w-60 absolute left-0 bottom-full mb-1 w-max p-2 bg-gray-700 text-white text-xs rounded-md transition-opacity duration-200">
              {t('REQUEST.AUTH.OAUTH2.AUTO_FETCH_TOKEN_HINT')}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 w-full">
        ...
        />
        <label className={`block min-w-[140px] ${isAutoRefreshDisabled ? 'text-gray-500' : ''}`}>{t('REQUEST.AUTH.OAUTH2.AUTO_REFRESH_TOKEN')}</label>
        <div className="flex items-center gap-2">
          <div className="relative group cursor-pointer">
            <IconHelp size={16} className="text-gray-500" />
            <span className="group-hover:opacity-100 pointer-events-none opacity-0 max-w-60 absolute left-0 bottom-full mb-1 w-max p-2 bg-gray-700 text-white text-xs rounded-md transition-opacity duration-200">
              {t('REQUEST.AUTH.OAUTH2.AUTO_REFRESH_TOKEN_HINT')}
            </span>
          </div>
        </div>
      </div>
