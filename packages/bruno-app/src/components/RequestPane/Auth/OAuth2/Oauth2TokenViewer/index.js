    ...
    <div className="mb-2 border dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      {isExpanded && (
        <div className="p-3">
          <div className="relative group">
            <div className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                title={t('REQUEST.AUTH.OAUTH2.COPY_TOKEN')}
              >
                {copied
          ...
          {decodedToken && (
            <div className="mt-3">
              <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">{t('REQUEST.AUTH.OAUTH2.DECODED_PAYLOAD')}</div>
...
const ExpiryTimer = ({ expiresIn }) => {
  ...
  return (
    ...
    >
      {timeLeft > 0 ? t('REQUEST.AUTH.OAUTH2.EXPIRES_IN', { time: formatExpiryTime(timeLeft) }) : t('REQUEST.AUTH.OAUTH2.EXPIRED')}
    </div>
  );
};
  ...
  return (
    <StyledWrapper className="relative w-auto h-fit mt-2">
      {Object.keys(creds)?.length ? (
        creds?.error ? (
          <pre className="text-red-600 dark:text-red-400">{t('REQUEST.AUTH.OAUTH2.ERROR_FETCHING_TOKEN')}</pre>
        ) : (
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
            <TokenSection title={t('REQUEST.AUTH.OAUTH2.ACCESS_TOKEN_LABEL')} token={creds.access_token} />
            <TokenSection title={t('REQUEST.AUTH.OAUTH2.REFRESH_TOKEN_LABEL')} token={creds.refresh_token} />
            <TokenSection title={t('REQUEST.AUTH.OAUTH2.ID_TOKEN')} token={creds.id_token} />
            {(creds.token_type || creds.scope) ? (
              <div className="mt-3 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg text-xs">
                <div className="grid grid-cols-2 gap-2">
                  {creds.token_type ? (
                    <div className="flex items-center space-x-1">
                      <span className="font-medium">{t('REQUEST.AUTH.OAUTH2.TOKEN_TYPE')}:</span>
                      <span className="text-gray-600 dark:text-gray-300">{creds.token_type}</span>
                    </div>
                  ) : null}
                  {creds?.scope ? (
                    <div className="flex items-center space-x-1 min-w-0">
                      <span className="font-medium flex-shrink-0">{t('REQUEST.AUTH.OAUTH2.SCOPE_LABEL')}:</span>
        ...
        <div className="text-gray-500 dark:text-gray-400">{t('REQUEST.AUTH.OAUTH2.NO_TOKEN_FOUND')}</div>
      )}
    </StyledWrapper>
  );
};
