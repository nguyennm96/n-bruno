  ...
  return (
    <Modal onClose={onClose} handleCancel={onClose} title={t('COOKIES.CLEAR_DOMAIN_COOKIES')} hideFooter={true}>
      ...
      <div className="font-normal mt-4">
        {t('COOKIES.CONFIRM_CLEAR_DOMAIN', { domain })}
      </div>
...
const DeleteCookieModal = ({ onClose, cookieName, onDelete }) => {
  ...
  return (
    <Modal onClose={onClose} handleCancel={onClose} title={t('COOKIES.DELETE_COOKIE')} hideFooter={true}>
      ...
      <div className="font-normal mt-4">
        {t('COOKIES.CONFIRM_DELETE_COOKIE', { name: cookieName })}
      </div>
...
const CollectionProperties = ({ onClose }) => {
  ...
  const clearDomainCookiesAction = () => {
    dispatch(deleteCookiesForDomain(domainToClear))
      .then(() => {
        toast.success(t('COOKIES.DOMAIN_COOKIES_CLEARED'));
      })
      .catch((err) => console.log(err) && toast.error(t('COOKIES.FAILED_TO_CLEAR_DOMAIN')));
  ...
  const deleteCookieAction = () => {
    if (cookieToDelete) {
      ...
      dispatch(deleteCookie(domain, path, key))
        ...
        .catch((err) => console.log(err) && toast.error(t('COOKIES.FAILED_TO_DELETE_COOKIE')));
    }
        ...
        <StyledWrapper>
          {!cookies || !cookies.length ? (
            ...
            <div className="flex items-center justify-center flex-col">
              ...
              <p className="empty-text mt-2">{t('COOKIES.ADD_COOKIES_TO_GET_STARTED')}</p>
              <Button
                type="submit"
                size="sm"
                className="mt-8"
            ...
            <div className="flex items-center justify-center flex-col">
              <IconSearch size={48} />
              <h2 className="text-lg font-medium mt-4">{t('COOKIES.NO_SEARCH_RESULTS')}</h2>
                    ...
                    <Accordion.Header index={i} className="flex items-center">
                      <div className="flex items-center">
                        ...
                        <span className="domain-count ml-2 text-xs">
                          ({t('COOKIES.COOKIE_COUNT', { count: domainWithCookies.cookies.length })})
                        </span>
                        ...
                        <table className="w-full">
                          <thead>
                            <tr className="text-left">
                              <th className="py-2 px-4 font-medium w-32">{t('COMMON.NAME')}</th>
                              <th className="py-2 px-4 font-medium w-52">{t('COMMON.VALUE')}</th>
                              ...
                              <th className="py-2 px-4 font-medium text-right w-24">{t('COMMON.ACTIONS')}</th>
                            </tr>
                          </thead>
                          <tbody>
