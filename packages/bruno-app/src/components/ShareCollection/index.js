...
const ShareCollection = ({ onClose, collectionUid }) => {
  ...
  const handleExportZip = async () => {
    try {
      ...
      toast.error(t('SHARE_COLLECTION.EXPORT_FAILED') + error.message);
    }
  };
    ...
    <Modal size="lg" title={t('SHARE_COLLECTION.TITLE')} handleCancel={onClose} hideFooter>
      <StyledWrapper className="flex flex-col">
        <p className="text-sm mb-4">
          ...
          {' '}{t('SHARE_COLLECTION.OPENCOLLECTION_DESC')}
        </p>

        {/* AhaMan Format Section */}
        <div className="section-title">{t('SHARE_COLLECTION.AHAMAN_FORMAT')}</div>
        <div className="bruno-format-grid mb-6">
          ...
          >
            ...
            <p className="card-description">{t('SHARE_COLLECTION.ZIP_DESC')}</p>
          ...
          >
            ...
            <p className="card-description">{t('SHARE_COLLECTION.YAML_DESC')}</p>
            <div className="feature-list">
              <div className="feature-item">
                <IconCheck size={14} className="checkmark" />
        ...
        <div className="other-format-grid">
          ...
          >
            ...
            <div className="format-info">
              ...
              <div className="format-description">{t('SHARE_COLLECTION.POSTMAN_DESC')}</div>
            </div>
          </div>
        </div>
        ...
        {selectedFormat === EXPORT_FORMATS.POSTMAN && hasNonExportableRequestTypes.has && (
          <div className="flex items-center mt-4 p-3 rounded" style={{ backgroundColor: 'rgba(251, 191, 36, 0.1)' }}>
            ...
            <span className="text-sm" style={{ color: '#f59e0b' }}>
              {t('SHARE_COLLECTION.NON_EXPORTABLE_WARNING', { types: hasNonExportableRequestTypes.types.join(', ') })}
            </span>
          </div>
        )}
