...
const CreateExampleModal = ({ isOpen, onClose, onSave, title = 'Create Response Example', initialName = '' }) => {
  ...
  const handleConfirm = () => {
    if (name.trim()) {
      ...
      setNameError(t('RESPONSE_EXAMPLE.CREATE_MODAL.NAME_REQUIRED'));
    }
  };
  ...
  return (
    <Portal>
      <Modal
        ...
        confirmText={t('RESPONSE_EXAMPLE.CREATE_MODAL.CONFIRM')}
        cancelText={t('RESPONSE_EXAMPLE.CREATE_MODAL.CANCEL')}
        isOpen={isOpen}
      >
        <div className="space-y-4">
          <div>
            <label htmlFor="exampleName" className="block font-medium">
              {t('RESPONSE_EXAMPLE.CREATE_MODAL.NAME_LABEL')}<span className="text-red-600">*</span>
            </label>
            <input
              id="exampleName"
              type="text"
          ...
          <div>
            <label htmlFor="exampleDescription" className="block font-medium">
              {t('RESPONSE_EXAMPLE.CREATE_MODAL.DESCRIPTION_LABEL')}
            </label>
            <textarea
